...
NOTES = "label smoothing 0.05, batch size 64 for CPU throughput"
...
LOG_COLUMNS = ["run_id", "timestamp", "top1_val_acc", "time_budget_sec", "notes"]
...
def load_splits() -> tuple[pd.DataFrame, pd.DataFrame, list[str]]:
    """Load cached splits from prepare.py. Returns (train_df, val_df, label_names)."""
    ...
    with open(CACHE_PKL, "rb") as f:
        data = pickle.load(f)
    return data["train_df"], data["val_df"], data["label_names"]
...
def main():
    ...
    train_df, val_df, label_names = load_splits()
    ...
    for stage_idx, stage in enumerate(STAGES):
        ...
        for epoch in range(1, max_epochs + 1):
            ...
            train_loss, train_acc, epoch_timed_out = train_one_epoch(
                model, train_loader, criterion, optimizer, device, scheduler, deadline
            )
